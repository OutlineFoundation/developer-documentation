...
1.  Reach out to the Psiphon team to obtain a config that gives you access to
    their network. This may require a contract.
2.  Add the received Psiphon config to the `fallback` section of your Smart
...
Fallback strategies are used when none of the proxyless strategies are
successful. They are typically proxies that are expected to be more reliable.
...
For example, this is how you can register a fallback that is configured with 
`{error: "my error message"}` that always returns an error on dial:
