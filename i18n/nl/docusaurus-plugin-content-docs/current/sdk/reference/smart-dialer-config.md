...
- `override:host=cloudflare.net|tlsfrag:1` geeft bijvoorbeeld een transport aan dat domain fronting gebruikt met Cloudflare en TLS-fragmentatie. In de [configuratiedocumentatie](https://pkg.go.dev/golang.getoutline.org/sdk/x/configurl#hdr-Config_Format) vind je meer informatie.
...
- Een geldige `StreamDialer`-configuratietekenreeks zoals beschreven in [`configurl`](https://pkg.go.dev/golang.getoutline.org/sdk/x/configurl#hdr-Proxy_Protocols).
...
:::note
De Psiphon-codebase is gelicentieerd onder de GPL, waardoor er licentiebeperkingen kunnen gelden voor je eigen code. Je kunt overwegen om een speciale licentie bij ze aan te vragen.
:::
